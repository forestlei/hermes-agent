...
## Alternative: inference.sh CLI (from twitter-automation)

For users who prefer a simpler setup without managing 5 X API credentials, the `infsh` CLI from inference.sh provides an alternative path.

**Install**: `infsh login`

**Available apps**: `x/post-tweet`, `x/post-create` (with media), `x/post-like`, `x/post-retweet`, `x/dm-send`, `x/user-follow`, `x/user-get`, `x/post-delete`, `x/post-get`

**Example**: `infsh app run x/post-tweet --input '{"text": "Hello!"}'`

**Trade-off**: Easier setup (no 5 API keys), but depends on inference.sh as intermediary. Less control than direct API. Use this when you want quick X automation without the developer portal friction.

**Workflow: AI content + X posting**: Generate image with `falai/flux-dev-lora` → post with `x/post-create` including `media_url`.
