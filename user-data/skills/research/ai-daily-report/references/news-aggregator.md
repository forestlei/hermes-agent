...
### Toutiao Article Fetching (from toutiao-article-fetcher)

Specialized fetcher for Toutiao (今日头条) mobile article URLs shared via WeChat.

**Key patterns**: 
- Use mobile User-Agent for Toutiao URLs
- Extract metadata from `og:title` (MORE RELIABLE than ld+json title)
- `/w/` format pages: content behind app wall — use OCR fallback or arXiv cross-reference
- Always verify GitHub/arXiv claims before scoring

**Scoring rubric**: depth×0.35 + originality×0.30 + accuracy×0.20 + readability×0.15 (0-100). Survey papers get +10~15 bonus.

**Marketing detection red flags**: Hyperbolic titles, "开源" claimed but no repo found after 3+ searches (accuracy ≤25), "核心功能推测" = SEO spam (≤25), vendor changelogs with zero analysis (≤30).

**Source registry**: `~/.hermes/sources/registry.json` — add sources with score ≥80.

**Batch processing**: Single fetch call for all URLs → single score call → single registry update. 3-call pattern is ~5x faster.

**Integration**: Bridges to ai-daily-report's source registry. TAF score ≥80 → eligible for ADR auto-collection.

### Multi-Search Engine (from multi-search-engine)

Integration of 17 search engines (8 CN + 9 Global) for web crawling without API keys.

**CN engines**: Baidu, Bing CN, Bing INT, 360, Sogou, WeChat (Sogou), Toutiao, Jisilu
**International engines**: Google, Google HK, DuckDuckGo, Yahoo, Startpage, Brave, Ecosia, Qwant, WolframAlpha

**Advanced operators**: `site:`, `filetype:`, exact match `""`, exclude `-`, OR. Time filters: `tbs=qdr:h/d/w/m/y`.

**Privacy engines**: DuckDuckGo (no tracking), Startpage (Google results + privacy), Brave (independent index), Qwant (EU GDPR).

**DuckDuckGo Bangs**: `!g` (Google), `!gh` (GitHub), `!so` (Stack Overflow), `!w` (Wikipedia).

**WolframAlpha**: Math, conversions, stocks, weather queries.
