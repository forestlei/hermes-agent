...
## PEFT / LoRA Fine-Tuning (from peft-fine-tuning skill)

GRPO training uses PEFT (LoRA) as its default parameter-efficient approach. The PEFT library provides:

- **LoRA** (Low-Rank Adaptation): Adds trainable rank-decomposition matrices to attention/MLP weights. Train <1% of parameters while maintaining full model quality.
- **QLoRA**: LoRA + 4-bit quantization for even lower memory usage.
- **AdaLoRA**: Adaptive rank allocation — allocates more parameters to important weight matrices.

**Common LoRA config for GRPO**: `r=16`, `lora_alpha=32`, targeting `q_proj, k_proj, v_proj, o_proj, gate_proj, up_proj, down_proj`.

**Full PEFT reference → `references/peft-fine-tuning.md`**
