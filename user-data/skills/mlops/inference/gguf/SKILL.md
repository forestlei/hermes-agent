...
## Constrained / Guided Generation (from guidance skill)

When deploying quantized GGUF models, structured output is often critical. The `guidance` library and llama.cpp's grammar feature enable JSON-schema-constrained generation, regex-constrained output, and selective generation — ensuring quantized models produce valid structured data.

**Key approaches:**
- **llama.cpp grammars**: Native GBNF grammar support for constrained generation at inference time (`--grammar` flag)
- **Guidance library**: Python-based prompt templating with `{{#select}}`, `{{gen}}`, and JSON-schema enforcement
- **Outlines**: Alternative structured generation library compatible with llama-cpp-python

**When to use:** API response formatting, function-call argument generation, structured data extraction from quantized models.

**Full reference → `references/guidance-constrained-generation.md`**
