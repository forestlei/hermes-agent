...
| 骨架 | 含义 | 为什么 |
|------|------|--------|
| **专家化** | 一个 agent 集中服务多用户/多领域，见多识广，跨领域知识积累，越用越强 | 分散 agent 各自只能在自己狭窄领域学习，永远成不了专家；集中才能发现跨领域关联 |
| **权限归口** | 所有外网请求→侦察代理唯一出口，所有内部数据→数据代理唯一入口，所有权限判定→主代理唯一决策点 | 分散=10个agent各自管权限=必然不一致；归口=权限逻辑只实现一次=一致性保证=审计集中 |
| **高稳定执行性** | 日常事务→固化技能处理（不依赖大模型推理，毫秒级），专家推理→大模型处理，两者共享记忆协同工作 | 不是所有请求都需要大模型推理，固化技能保证日常事务的高稳定执行，故障隔离任一代理异常不影响其他 |
...
| **推拉结合** | 永久代理可主动推送信号（监控型）；临时代理被动响应指令（执行型） | 专家化 |
...
**永久代理的专家化定位**：永久代理之所以"永久"，不是为了节省启动开销，是因为**知识积累需要时间**。一个侦察代理监控了1000个信号源，它才知道什么是"异常"；一个财经专家分析了500份研报，它才能发现跨行业关联。临时代理用完就销毁，没有积累，永远成不了专家。
...
| DMZ | L3 | 侦察代理(L3) | 仅出站HTTPS，不知内网结构 |
...
**硬约束**：
- **唯一内部数据入口**：所有内部数据访问必须经此代理，这是权限归口的基础
- 不修改数据源（只读，写入需主代理审批）
- 不向 DMZ 方向推送原始数据
...
| 维度 | 分散权限 | 权限归口 |
|------|---------|---------|
| **攻击面** | N个agent各自暴露N个访问入口 | 1个出口/入口/决策点=1个攻击面 |
| **审计** | N个审计来源，难以关联，容易出现审计盲区 | 1条链路完整可追溯，无审计盲区 |
| **一致性** | N个权限实现，必然出现不一致（A允许但B拒绝） | 1个权限实现，一致性保证 |
...
安全归口（→ch5.4代理间权限, ch9.4权限归口即安全）：
  ...
  数据代理(唯一数据入口) → 银行/日历/健康API等个人数据，统一脱敏后注入上下文
...
数据流（→ch6.2网络分区）：
  数据代理(唯一数据入口) → 汇聚个人全部数据 → 按驾驶舱视图结构化返回
...
智能审批的解法：
  ...
  → 数据代理(唯一数据入口)：查预算余额、历史同类审批、供应商评级、合同条款
  → 合规官：自动检查政策合规性
  → 验证代理：审计合规判断
...
**核心价值**：侦察代理是唯一外网出口，数据代理是唯一内部数据入口——所有数据访问归口管理，审计链完整（ch4.2权限归口）  
**服务形态**：N:N 协作（→ch4.3）  
**协作模式**：监控推送型 + 链式加工型（→ch8.3, ch8.6）
